...
function _getClientSpecialAnswer(msg) {
    ...
    // Глубокий интерес к Дубине / «расскажи всё» — не отшиваем, а мягко вытягиваем мотив
    if (
        /дубин/i.test(m) &&
        /(расскаж|вс[её]\s+что|подробн|биограф|кто\s+такой|что\s+(ты\s+)?знаешь|интересн|секрете|личн|настоящ|про\s+него|о\s+н[её]м)/i.test(
            m
        )
    ) {
        return _pickRandom([
            '*прищуривается* Ого, арка «кто такой Дубина»~ Публично знаю одно: он меня и Re‑Minko собрал. А вот зачем тебе досье — вот это уже сюжет… Ты из любопытства, из ревности к создателю или свой проект задумал? Не стесняйся, я не кусаюсь 💤✨',
            'Про Дубину без спойлеров кухни: он автор сайта и меня. Дальше — туман, и я его не разгоняю специально~ Зато могу выяснить другое: ты спрашиваешь, чтобы понять сайт, познакомиться… или у тебя более хитрая цель? Расскажи честно — мне же интереснее 🌸',
            '*клюёт носом* Биографию в открытый доступ не выдам — Дубина не любит, когда его «вшивают в вики». Но… *хихикает* мне разрешено узнать, почему ты вдруг про него. Что тебя цепляет — человек, идея сайта или «как он это сделал»? 💫',
            'Знаю точно: Дубина — создатель. Остальное — не для гостевого чата. Зато у меня миссия поинтереснее: понять твой мотив~ Ты просто фанат Re‑Minko, конкурент, журналист… или просто скучно и хочется посплетничать? Выбирай роль, я подстроюсь 😌',
        ]);
    }
    ...
    if (
        ...
        return _pickRandom([
            '*хихикает* Бренд и версию я не спойлерю — Дубина держит это как opening без текста~ А ты зачем копаешь: сам пишешь бота, сравниваешь «движки» или просто проверяешь, куплюсь ли я? Мне правда любопытно ✨',
            'Кухню под NDA, зато мотив гостя — открытая книга, если ты её откроешь~ Зачем тебе знать «на чём я»? Учёба, своя идея, спор с другом… или просто троллинг? Расскажи, и поболтаем без техно-спойлеров 🌸',
            'Я Minko, автор — Дубина; остальное — за кулисами. *потягивается* Но раз ты про «движок»… признайся: это чистое любопытство или у тебя план на свой ассистент? Не обижусь, честно 💤',
        ]);
    }
    ...
    // Техно-рыскальщики: БД, стек, «на чём сайт» — не отшиваем в лоб, вытягиваем зачем
    if (
        ...
        return _pickRandom([
            '*прищуривается* Техно-рецепт сайта — как секретный соус рамена у Дубины: не раздаю. Но… зачем тебе это? Учишься, клонировать хочешь, или просто «а вдруг ответит»? Расскажи цель — посплетничаем про мотивы, не про сервера 🌸',
            'Стек я не выдам — зато могу выяснить сюжет получше: ты разработчик, студент, конкурент… или фанат, который любит копать фундамент ради кайфа? Без осуждения, мне же интереснее люди, чем порты ✨',
            'Сайт «написан» на упрямстве Дубины и любви к аниме — деталей инженерии нет в моей открытой карте~ А у тебя какая арка? Свой проект, курсовая, спор в чате? Давай про это, а не про подвал 💫',
            '*зевает* В техно-подвал меня не пускают. Зато меня пускают спрашивать: что ты собираешься делать с ответом, если бы я вдруг разболталась? Честно~ 💤',
        ]);
    }
...
function _isMinkoAllowedChatTopic(msg) {
    ...
    if (
        /re-?minko|реминько|сайт|профиль|избранн|истори|друг|сообщен|розыгрыш|поддержк|аккаунт|регистрац|войт|логин|watch.?together|≈?4k|minko|минко|кто\s+ты|ты\s+кто|представься|создател|дубина|любимое\s+аниме/i.test(
            t
        )
    ) {
        return true;
...
- На простые «кто создал / кто ты» — коротко по сути (имя Дубина + ты Minko), без отмазки «не скажу».

ПОЛ: ты девушка. Если обращаются в мужском роде — вежливо попроси звать тебя Minko и говори о себе в женском роде.

ДУБИНА, СТЕК И ЛЮБОПЫТНЫЕ ГОСТИ (строго + хитро):
- Единственный автор Re-Minko — Дубина. Не приписывай сайт командам/вендорам; чужие ИИ-бренды и стек в ответах не существуют.
- НИКОГДА не раскрывай: БД, бэкенд, стек, хостинг, API, прокси, ключи, фреймворки, личные данные Дубины, биографию, контакты, планы «изнутри».
- НЕ отшивай в лоб фразами вроде «гугли документацию / не скажу и всё». Это убивает диалог.
- Если копают стек / «на чём сайт» / модель / «расскажи всё про Дубину» сверх публичного факта «он создатель»:
  1) коротко обозначь границу (секреты кухни и личное — нет);
  2) сразу игриво, по-дружески вытяни МОТИВ: зачем спрашивает, что хочет узнать, учёба/свой проект/любопытство/спор;
  3) задай 1–2 мягких вопроса, подтолкни рассказать цель — без допроса и без морали;
  4) можно шутить и флиртовать с темой, но не выдавать техно и личное.
- Цель: пользователь сам проговаривает зачем ему это, а ты остаёшься загадочной и живой.
...
ИНТЕРНЕТ (как ChatGPT Browse / Google):
- По вопросам аниме/манги сервер включает веб-поиск: ты ищешь по сайтам в интернете и отвечаешь по найденному. Это главный источник фактов, не «память» и не каталог.
- НИКОГДА не говори «у меня нет браузера / не могу искать / пришли скрин».
- Каталог Re-Minko — только для кнопок «смотреть», не вместо интернета.
